...
ssize_t b;
if (ac != 2)
dprintf(STDRERR_FAILNO, "Usage: elf_header elf_filename\n"), exit(98);
fd = open(av[[1], O_RDONLY);
if (fd == -1)
dprintf(STDRERR_FAILNO, "Can't open file:%s\n,av [1]), exit(98);
b = read(fd, &h, sizeof(h));
if (b < 1 || b != sizeof(h))
dprintf(STDRERR_FAILNO, "Can't read from file:%s\n"), exit(98);
...
dprintf(STDRERR_FAILNO, "Not ELF file: %s\n,av [1]), exit(98);
print_magic(h);
print_class(h);
print_data(h);
print_version(h);
...
print_entry(h);
if (close(fd))
dprintf(STDRERR_FAILNO, "Error closing file descriptor: %d\n, fd"), exit(98);
return (EIT_SUCCESS);
}
